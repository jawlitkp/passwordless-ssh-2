...
def set_ssh_pexpect(host, logger, username, password, port=22, timeout=30):
    ...
    try:
        ...
        p = pexpect.spawn('ssh-copy-id', args, timeout=timeout)
        match = p.expect([ssh_newkey, '(?i)password:'])
	    ...
	    #match = p.expect(pexpect.EOF)
            #print returnCode
            #p.expect([ssh_newkey, '#'])
            if returnCode != 4:
                logger.error("ssh-cpy-id failed!")
                raise
            elif ssh_error in p.before.strip() in ssh_error:
                raise(ssh_error)
        logger.debug("expect output: %s" % p.before)
        # Last check if SSH is actually working at the end
        (outp, rc) = pexpect.run("ssh -nx " + host + " -p " + str(port) + " ls", timeout=timeout, withexitstatus=True)
        if rc:
...
def main():
    ...
    parser.add_argument("-U", "--username", help="Login Username", default="", type=str)
    parser.add_argument("-P", "--password", help="Login Password", default="", type=str)
    ...
    if not is_hostname(args.host):  # checking if hostname or IP is given
        if is_ipv4(args.host):  # checking if it is valid IPV4 address
            ...
            (outp, rc) = pexpect.run("ssh " + args.host + " -p " + str(args.port)  + " ls", timeout=args.timeout, withexitstatus=True)
            if rc:
            ...
            (outp, rc) = pexpect.run("ssh -6 " + args.host + " -p " + str(args.port) + " 'ls'", timeout=args.timeout, withexitstatus=True, events={"(?i)are you sure you want to continue connecting":"yes\\n"})
            if rc:
                logger.info("Failed to set SSH for " + args.host)
                raise Exception
            else:
